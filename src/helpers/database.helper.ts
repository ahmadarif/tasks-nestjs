...
export const getTypeOrmModuleOptions = (type: string, sid: string) => {
  ...
  switch (type) {
    ...
    case 'cockroachdb':
      ...
      if (Config.get('DB_SSL_CA') && Config.get('DB_SSL_KEY') && Config.get('DB_SSL_CERT')) {
        config = {
          type: 'cockroachdb',
          ssl: {
            rejectUnauthorized: false,
            ca: readFileSync(Config.get('DB_SSL_CA')).toString(),
            key: readFileSync(Config.get('DB_SSL_KEY')).toString(),
            cert: readFileSync(Config.get('DB_SSL_CERT')).toString(),
          },
        };
      }
    break;
